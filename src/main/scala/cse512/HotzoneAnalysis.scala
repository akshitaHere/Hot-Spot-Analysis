...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    pointDf.createOrReplaceTempView("temppoint")

    // Parse point data formats
    spark.udf.register("trim",(string : String)=>(string.replace("(", "").replace(")", "")))
    pointDf = spark.sql("select trim(_c5) as _c5 from temppoint")
    ...
    val countRectangle = joinDf.groupBy("rectangle").count.orderBy("rectangle").coalesce(1)

    return countRectangle // YOU NEED TO CHANGE THIS PART
  }
